...
| `contour_type` — Nozzle contour / Nozzle profili | Conic: a straight-walled cone at the divergent half-angle below. Bell: an approximated contoured (Rao-style) nozzle, shorter and slightly more efficient for the same expansion ratio. | Konik: aşağıdaki ıraksak yarım açıyla düz duvarlı bir koni. Bell: yaklaşık (Rao tipi) profilli nozzle, aynı genişleme oranı için daha kısa ve biraz daha verimli. |
...
| `core_diameter` — Core Ø / Çekirdek Ø | Diameter of the central bore (star: the root/valley diameter; wagon-wheel: the hub diameter; rod-and-tube: the rod diameter). A wider core lowers pressure and thrust but shortens burn time. | Merkezî deliğin çapı (yıldızda: kök/vadi çapı; tekerlekte: göbek çapı; çubuk-tüpte: çubuk çapı). Geniş çekirdek basıncı ve itkiyi düşürür ama yanma süresini kısaltır. |
...
| `grain_type` — Grain geometry / Grain geometrisi | How the propellant burns. BATES is neutral and recommended; a tube is progressive and pushes pressure up during the burn; star and wagon-wheel start high and taper off; rod-and-tube can be tuned close to flat by balancing the rod and tube webs. | Yakıtın nasıl yandığı. BATES nötrdür ve önerilir; tüp progresiftir ve yanma boyunca basıncı yükseltir; yıldız ve tekerlek yüksek başlayıp azalır; çubuk-tüp, çubuk ve tüp yanma paylarını dengeleyerek neredeyse düz yapılabilir. |
...
| `n_points` — Point/slot count / Uç/yuva sayısı | Number of star points or wagon-wheel slots. | Yıldız ucu ya da tekerlek yuvası sayısı. |
...
| `nozzle_material` — Nozzle material / Nozzle malzemesi | The nozzle's own material, for its mass and the technical drawing. Leave on “same as case” unless you are machining it from something else. | Nozzle'ın kendi malzemesi; kütlesi ve teknik çizim için kullanılır. Başka bir şeyden işlemiyorsan “case ile aynı” bırak. |
...
| `point_diameter` — Point/slot Ø / Uç/yuva Ø | Star/wagon-wheel: diameter reached by the point tips or slot tips. Rod-and-tube: the tube's own inner bore diameter. Must sit between the core diameter and the grain OD. | Yıldız/tekerlek: uçların veya yuvaların ulaştığı çap. Çubuk-tüp: tüpün kendi iç çapı. Çekirdek çapı ile grain dış çapı arasında olmalı. |
...
| `meop` — MEOP (case limit) / MEOP (case sınırı) | The pressure limit you told the program your case can take. If peak pressure exceeds it, export is locked until you accept the risk. | Case'inin kaldırabileceğini söylediğin basınç sınırı. Tepe basınç bunu aşarsa, riski kabul edene kadar dışa aktarım kilitlenir. |
...
| `peak_pressure_eroded` — Peak pressure (with erosion) / Tepe basınç (erozyonlu) | Highest chamber pressure with throat erosion applied - the throat opening up over the burn lowers this below the erosionless value above. Structural safety is still judged against the erosionless peak. | Boğaz erozyonu uygulanmış en yüksek oda basıncı - yanma boyunca boğazın genişlemesi bu değeri yukarıdaki erozyonsuz değerin altına düşürür. Yapısal güvenlik yine de erozyonsuz tepe basınca göre değerlendirilir. |
